...
转录原始视频。
Use Whisper's original segments as the editing unit.
使用 Whisper 原始 segment 作为选段单位。
Compare the transcript with the written script.
把视频转录文本和原始文字稿进行比对。
Search from the end of the footage backward, because the last repeated take is often the best one.
从视频后面往前找，因为重复录制时，最后一遍往往是更顺的版本。
Return selected clips in normal script order.
选完后再按文字稿正序排列。
Generate a Markdown/JSON review list before rendering any video.
